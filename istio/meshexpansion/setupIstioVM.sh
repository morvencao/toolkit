...
function istioInstall() {
  ...
  # add --force-bad-version option for daily build package
  dpkg --force-bad-version -i ${ISTIO_STAGING}/istio-sidecar.deb
  
  # change owner to 'istio-proxy'
  chown -R istio-proxy:istio-proxy /var/lib/istio
